...
// ============================================================
// Stockage persistant local (survit à la fermeture du navigateur)
// ============================================================
function usePersistentStore(storageKey, initial) {
...
export default function App() {
  const [clients, setClients] = usePersistentStore("tournee_clients", []);
  const [geoCache, setGeoCache] = usePersistentStore("tournee_geocache", {}); // cle "CP|VILLE" -> {lat, lon}
  const [planning, setPlanning] = usePersistentStore("tournee_planning", {}); // dateKey -> [{clientId, heureArrivee, heureFin}]
  const [departs, setDeparts] = usePersistentStore("tournee_departs", {}); // dateKey -> {adresse, coords, heure}
      ...
      <div className="tr-shell">
        <header className="tr-header">
          <div className="tr-title tr-font-display">
            Tournée
            <small>Prochain RDV optimal · Plan B en cas d'imprévu</small>
          </div>
