...
function PanneauGoogle({ icalUrl, onSave, googleEvents, onRefresh, syncing, syncError }) {
  ...
  const lbl = { display:"block", fontSize:11, textTransform:"uppercase", letterSpacing:"0.06em", color:"#8A93A0", marginBottom:5, fontWeight:600 };
  const inp = { width:"100%", padding:"9px 11px", border:"1.5px solid #DCD7CB", borderRadius:6, fontSize:11, fontFamily:"inherit", color:"#1C2630", background:"#F5F2EC", boxSizing:"border-box" };
      ...
      {!icalUrl ? (
        <>
          <p style={{ fontSize:12.5, color:"#8A93A0", marginBottom:10, lineHeight:1.5 }}>
            Colle ici ton <strong style={{color:"#1C2630"}}>adresse secrète iCal</strong> Google Agenda.
          </p>
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, flexWrap:"wrap" }}>
          <div style={{ fontSize:12.5, color: syncError ? "#C75450" : "#5B8C6E" }}>
            {syncError
              ? <><AlertCircle size={13} style={{ display:"inline", marginRight:5, verticalAlign:-2 }}/>Erreur de sync</>
...
export default function AgendaView({ planning, rdvParJourCalcule, agendaRdvs, setAgendaRdvs, codeSync }) {
  ...
  const fetchGoogle = useCallback(async (url) => {
    if (!url) return;
    setSyncing(true);
    setSyncError(false);
    try {
      // Appel de la Edge Function Supabase (côté serveur, pas de CORS)
      const res = await fetch(`${SUPABASE_URL}/functions/v1/sync-google-agenda`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({ icalUrl: url, code: codeSync || "tournee" }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error);

      // Charge les événements depuis Supabase
      const evRes = await fetch(`${SUPABASE_URL}/rest/v1/agenda_google?code=eq.${codeSync || "tournee"}&select=*`, {
  ...
  return (
    <div style={{ fontFamily:"'Inter',system-ui,sans-serif", color:"#1C2630" }}>
      ...
      {showConfig && <PanneauGoogle icalUrl={icalUrl} onSave={saveIcalUrl} googleEvents={googleEvents} onRefresh={()=>fetchGoogle(icalUrl)} syncing={syncing} syncError={syncError}/>}
